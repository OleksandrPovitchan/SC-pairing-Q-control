...
J_full_val = spectrum[5]

psi_state = read_dictionary_1['psi_last']
...
i = 0
while i < len(weights):
    if J_full_val[i] < precision:
        J_full_val[i] = 0
        zero_current_E.append(E_val[i])
        zero_current_eta.append(Eta_sq_val[i])
        zero_current_weight.append(weights[i])
    i += 1

black_eta = np.array(black_eta)
color_eta = np.array(color_eta)

energy_final = read_dictionary_1['Energy_evolution'][-1]
print(energy_final)
eta_sq_final = read_dictionary_1['Eta_sq_evolution'][-1]
...
exp_dict = {
    ...
    'zero_current_E': zero_current_E,
    'zero_current_eta': zero_current_eta,
    'zero_current_weight': zero_current_weight
}

np.save(f'UE-spectral-weights_L{L}.npy', exp_dict) 