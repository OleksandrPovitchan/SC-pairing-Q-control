...
for h_tag in h_ranges:
    for v_tag in v_ranges:
        file = np.loadtxt(f'./grid-AQC/processing/L8-AQC-eta_sq_{mode}-{h_tag}-(0.1)_{v_tag}-(0.1).txt')
        if i%2 == 0: 
            v_array = file
        else: 
...
plot_array = np.real(h_array)
eta_max = np.max(h_array)
print(eta_max)
val_max = np.max(plot_array)

fig, (ax1) = plt.subplots(1, 1, figsize=(8, 4))


contour1 = ax1.contourf(X, Y, plot_array)
...
annotate_position = val_max
cbar.ax.annotate(f'{val_max:.3f}', xy=(1, annotate_position), xytext=(2.5, annotate_position),
                 arrowprops=dict(arrowstyle="-", color='black', linestyle = 'dotted'), color='black', va='center', fontsize=fontsize)
...
np.save(output_path + f'/global_AQC_dict.npy', dictionary) 
