...
pc_2, = ax2.plot(joint_times, joint_2/L, label = r'$\rm{LC^{(\downarrow)}}$')
pc_3, = ax2.plot(joint_times, joint_3/L, label = r'$\rm{LC^{(\downarrow*)}}$')
...
ax1.plot(joint_times, phi_joint_3, color = pc_3.get_color(), alpha = 1, linewidth = 0.5, label = r'$\Phi_{LC^{*}}$')
...
ax1.set_ylabel(r'$\Phi^{\rm{LC^{(\downarrow *)}}}(t)$', fontsize = fontsize, c = pc_3.get_color())
...
fig.savefig(path + f'/fig-5-a_L{L}.pdf', format='pdf', bbox_inches='tight', dpi=dpi)
...
axx2.plot(times_2, eta_final/L, color = 'red', label = r'$\rm{LC^{(\downarrow)(sat.)}}$')
...
axx2.axhline(loc_min_2, color = pc_2.get_color(), linestyle = (0, (2, 3)), linewidth = 3, label = r'$\rm{LC^{(\downarrow)}}$')
axx2.axhline(loc_min_3, color = pc_3.get_color(), linestyle = (0, (2, 3)), linewidth = 3, label = r'$\rm{LC^{(\downarrow*)}}$')
...
figg.savefig(path + f'/fig-5-b_L{L}.pdf', format='pdf', bbox_inches='tight', dpi=dpi)
